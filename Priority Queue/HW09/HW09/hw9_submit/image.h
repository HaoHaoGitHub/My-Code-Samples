...
class DistancePixel {
 public:
  DistancePixel() : x(-1),y(-1),distance(std::numeric_limits<double>::max()), is_black(false) {}
  // Another constructor function that takes into three variables
  DistancePixel(int _x, int _y, int _distance, bool condition) {
      x = _x;
      y = _y;
      distance = _distance;
      is_black = condition;
  ...
  bool get_black() const { return is_black; }
  // modifier
  void setX(int _x) { x = _x; }
  void setY(int _y) { y = _y; }
  void setValue(double v) { distance = v; }
  void set_black(bool condition) { is_black = condition;}

...
class DistancePixel_PriorityQueue {
 ...
 public:
  ...
  // A print function that is used for debugging:
  void print() {
  ...
  void percolate_down(int i) {
    // ASSIGNMENT: Implement this function
      ...
      while (i <= last_non_leaf()) {
          // If there is no children, then we don't need to percolate down:
          if (! has_left_child(i)) break;
          temp = get_left_child(i);
          if (has_right_child(i)) {
              if (*m_heap[get_right_child(i)] < *m_heap[get_left_child(i)]) {

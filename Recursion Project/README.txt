...
< Taoran Li, Aritra B, Steven, Xu Chai/ google, cplusplus >
...
ESTIMATE OF # OF HOURS SPENT ON THIS ASSIGNMENT:  < 65 >
...
can be placed. In the recursion function, I have two check functions: the first one (Chech_the_whole_board) is used for checking the whole layout of
the board after all the tiles have been used up, the second one(Check_tile) is used for checking the requirements of the current tile.
Then, after the recursion function, I have a “ check_duplicate code “, which is used for checking whether two solutions are the
same. The order notations of each part are as follows:
Order notations:
